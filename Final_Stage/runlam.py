...
def AngCal(image, cap, car):
    ...
    if(STRAIGHT_FLAG==1):
        STRAIGHT_COUNTER = STRAIGHT_COUNTER + 1
    if(STRAIGHT_COUNTER>60):
        STRAIGHT_FLAG=0
        STRAIGHT_COUNTER=0
        LIST_SIGN = []
    print("Len list sign: ", len(LIST_SIGN))
    if(len(LIST_SIGN)>3):
        print("most frequent list sign: ", most_frequent(LIST_SIGN))
    if(len(LIST_SIGN)>3 and (most_frequent(LIST_SIGN)==6 or most_frequent(LIST_SIGN)==2 or most_frequent(LIST_SIGN)==8)):
        STRAIGHT_FLAG = 1
        # if (angle<-10):
        #     angle = -3


    return -angle/1.2, speed, 0


def control(car, speed, angle, flag):
    if(flag==1):
        car.setSpeed_cm(0)
        car.setMotorMode(1)
        car.setMotorMode(0)
...
if __name__ == "__main__":
    ...
    while 1:
        ...
        angle, speed, flag = AngCal(image=result, cap=cap, car=Car)
        control(car=Car, speed=speed, angle=angle, flag=flag)
        ...
        print("Straight counter: ", STRAIGHT_COUNTER)
        print("Straight flag: ", STRAIGHT_FLAG)
